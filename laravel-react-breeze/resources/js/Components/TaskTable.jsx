...
export default function TaskTable() {
    const { removeTask, updateTasks, updateStatus, tasks, filterTasksByStatus, filterOption } = UseTask()
    const select = useRef(null)

    const handleClick = () => {
        filterTasksByStatus(select.current.value)
    }

    const filteredTasksToShow = filterOption === 'all'
        ? tasks
    ...
    return (
        <>
            <div className="mb-5">
                <select ref={select} onChange={handleClick} defaultValue={'all'} id="filter" name="filter" className="w-full px-4 py-2 border border-gray-300 bg-white text-gray-800 rounded-md focus:outline-none focus:ring focus:ring-indigo-200">
                    ...
                    {filteredTasksToShow.length === 0 ? (
                        ...
                        filteredTasksToShow.map(task => (
                            <tr key={task.id}>
                                ...
                                <td className={`px-6 py-4 whitespace-wrap bg-white text-black`}>{task.description}</td>
                                <td className={`px-6 py-4 whitespace-nowrap bg-white text-black`}>{task.author}</td>
                                <td className={`px-6 py-4 whitespace-nowrap bg-white text-black`}>
